    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading && <p className="text-sm text-zinc-600">Đang tải giỏ hàng...</p>}
          ...
          {!loading && hasItems && (
            <div className="space-y-4">
              {items.map((item) => {
                const selected = selectedItemIds.length === 0 || selectedItemIds.includes(item.cartItemId)
                  ...
                  <article key={item.cartItemId} className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                    <div className="flex items-start gap-3">
                      <input
                        type="checkbox"
                        checked={selected}
                        onChange={(event) => setItemSelected(item.cartItemId, event.target.checked)}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs text-zinc-600">{formatPrice(item.unitPrice)} / sản phẩm</p>
                        <p className="mt-1 text-sm font-semibold text-zinc-900">{formatPrice(item.lineTotal)}</p>

                        <div className="mt-3 flex items-center gap-2">
                          <button
                            type="button"
                            onClick={() => changeItemQuantity(item.cartItemId, Number(item.quantity || 1) - 1)}
                          ...
                          <span className="min-w-8 text-center text-sm font-semibold text-zinc-900">{item.quantity}</span>
                          <button
                            type="button"
                            onClick={() => changeItemQuantity(item.cartItemId, Number(item.quantity || 1) + 1)}
        ...
        <footer className="border-t border-zinc-200 p-5">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-lg font-semibold text-zinc-900">{formatPrice(selectedTotal)}</span>
          </div>
          <button
            type="button"
            disabled={!hasItems}
