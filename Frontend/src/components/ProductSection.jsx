...
function ProductSection() {
  ...
  return (
    <section id="products" className="mt-14 space-y-10">
      {!loading && !error && (topSellingProducts.length > 0 || topViewedProducts.length > 0) && (
        <section className="rounded-3xl border border-zinc-200 bg-white p-5">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500">Tổng quan</p>
              <h2 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-900">Top sản phẩm nổi bật</h2>
            </div>
            <p className="text-xs text-zinc-500">Tự động chuyển 3 giây mỗi sản phẩm</p>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {topSellingProducts.length > 0 && (
              <section className="rounded-2xl border border-zinc-200 bg-white p-4">
                <div className="mb-3 flex items-center justify-between gap-2">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">Top bán chạy</p>
                  <p className="text-xs text-zinc-500">5 sản phẩm bán nhiều nhất</p>
                </div>
                ...
                <div className="overflow-hidden">
                  <AnimatePresence mode="wait">
                    <motion.article
                      ...
                      onClick={() => navigate(`/products/${topSellingProducts[topSellingIndex]?.id}`)}
                    ...
                    >
                      ...
                      <p className="mt-1 text-xs text-zinc-600">
                        ...
                        <span className="font-semibold text-zinc-900">
                          {Number(topSellingProducts[topSellingIndex]?.soldCount || 0).toLocaleString('vi-VN')}
                        </span>
                      </p>
                      <p className="mt-1 text-sm font-semibold text-zinc-900">
                        {formatPrice(topSellingProducts[topSellingIndex]?.price)}
                      </p>
                    </motion.article>
                  </AnimatePresence>
                    ...
                    {topSellingProducts.map((product, index) => (
                      <button
                        ...
                        className={`h-2.5 w-2.5 rounded-full transition-colors ${
                          index === topSellingIndex ? 'bg-zinc-900' : 'bg-zinc-300 hover:bg-zinc-500'
                        }`}
            ...
            {topViewedProducts.length > 0 && (
              <section className="rounded-2xl border border-zinc-200 bg-white p-4">
                <div className="mb-3 flex items-center justify-between gap-2">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">Top lượt xem</p>
                  <p className="text-xs text-zinc-500">5 sản phẩm xem nhiều nhất</p>
                </div>
                ...
                <div className="overflow-hidden">
                  <AnimatePresence mode="wait">
                    <motion.article
                      ...
                      exit={{ opacity: 0, x: -24 }}
                      transition={{ duration: 0.35, ease: 'easeOut' }}
                      onClick={() => navigate(`/products/${topViewedProducts[topViewedIndex]?.id}`)}
                      className="relative mx-auto max-w-sm cursor-pointer overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50 p-3"
                    >
                      <span className="absolute right-2 top-2 rounded-full bg-zinc-900 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-white">
                      ...
                      <p className="mt-1 text-xs text-zinc-600">
                        Lượt xem:{' '}
                        <span className="font-semibold text-zinc-900">
                          {Number(topViewedProducts[topViewedIndex]?.viewCount || 0).toLocaleString('vi-VN')}
                        </span>
                      </p>
                      <p className="mt-1 text-sm font-semibold text-zinc-900">
                        {formatPrice(topViewedProducts[topViewedIndex]?.price)}
                      </p>
                    </motion.article>
                  </AnimatePresence>
                  ...
                  <div className="mt-3 flex items-center justify-center gap-1.5">
                    {topViewedProducts.map((product, index) => (
                      <button
                        ...
                        className={`h-2.5 w-2.5 rounded-full transition-colors ${
                          index === topViewedIndex ? 'bg-zinc-900' : 'bg-zinc-300 hover:bg-zinc-500'
                        }`}
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="mt-4 flex items-center justify-between gap-2">
                  ...
                  >
                    {Number(product?.stock || 0) <= 0 ? 'Hết hàng' : 'Thêm vào giỏ'}
                  </button>
                </div>
              </div>
            </motion.article>
