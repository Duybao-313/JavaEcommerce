    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#f7f7f4_0%,#f4f4ef_45%,#ffffff_100%)]">
      <header className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-8">
        ...
        <nav className="hidden gap-8 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 md:flex">
          ...
          <Link
            to={
              session?.token && session?.user?.role === "SELLER"
                ? "/seller"
                : "/products"
            }
          ...
          {session?.token ? (
            <Link to="/me" className="hover:text-zinc-900">
              Tài khoản
            </Link>
          ) : (
      ...
      <main className="mx-auto w-full max-w-6xl px-6 pb-20">
        ...
        >
          <motion.div style={{ y: heroTextY }}>
            ...
            <p className="mt-6 max-w-2xl text-base leading-relaxed text-zinc-600 md:text-lg">
              SplitGo tối ưu hành trình mua sắm bằng bố cục mạch lạc, thông điệp
              ngắn gọn và CTA nổi bật để tăng đơn hàng.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link
                to="/register"
                className="rounded-full border border-zinc-300 px-6 py-3 text-sm font-semibold text-zinc-800 hover:border-zinc-900"
              >
                Đăng ký
              </Link>
              <a
          ...
          >
            ...
            <p className="mt-2 text-3xl font-semibold leading-tight text-zinc-900">
              Giảm đến 40% cho danh mục hot
            </p>
            <p className="mt-3 text-sm leading-relaxed text-zinc-600">
              Ưu đãi giới hạn, cập nhật liên tục để tăng chuyển đổi và doanh
              thu.
            </p>
        ...
        <section
          id="features"
          className="mt-14 rounded-3xl border border-zinc-200 bg-zinc-950 p-8 text-zinc-100"
        >
          <h2 className="text-3xl font-semibold tracking-tight">
            Vì sao khách hàng chọn SplitGo
          </h2>
          <motion.ul
          ...
          >
            {highlights.map((item) => (
              <motion.li
                variants={fadeUpItem}
                key={item}
                className="rounded-2xl border border-zinc-700 bg-zinc-900 p-5 text-sm leading-relaxed"
              >
                {item}
              </motion.li>
            ))}
        ...
        <section
          id="contact"
          className="mt-14 rounded-3xl border border-zinc-200 bg-white p-8 md:p-10"
        >
          <h2 className="text-3xl font-semibold tracking-tight text-zinc-900">
            Nhận ưu đãi mới mỗi tuần
          </h2>
          <p className="mt-2 text-sm text-zinc-600">
            Đăng ký để nhận mã giảm giá, sản phẩm mới và chương trình độc quyền.
          </p>
          <form
